...
def transform_blocks_data(
    input_file_name: str, output_file_name: str, write_rows_at_once: bool = True
):
    """
    ...
    if write_rows_at_once:
        rows_to_write = []
        with open(input_file_name, "r") as infile:
            reader = csv.reader(infile)
            rows_to_write = [row[:-2] for row in reader]
...
def transform_transactions_data(
    ...
    if write_rows_at_once:
        ...
        with open(input_file_name, "r") as infile:
            reader = csv.reader(infile)
            rows_to_write = [row[:-3] for row in reader]
...
def transform_tokens_data(
    ...
    if write_rows_at_once:
        ...
        with open(input_file_name, "r") as infile:
            reader = csv.reader(infile)
            rows_to_write = [row[:-1] for row in reader]
